...
impl Plugin for RustPlugin {
    ...
    fn invoke_phase_b(&self, req: &InvokeRequest) -> InvokeResponse {
        use travsr_indexer::{sandbox::SandboxConfig, run_ra_lsif, ingest_rust};
        let cfg = SandboxConfig { repo_root: req.root.clone(), ..Default::default() };
        match run_ra_lsif(&req.root, &cfg) {
            Ok(Some(dump)) => match ingest_rust(&dump, "") {
                Ok(out) => InvokeResponse { nodes: out.nodes, edges: out.edges },
                Err(e) => { tracing::warn!("rust lsif ingest: {e}"); InvokeResponse::default() }
            },
            Ok(None) => { tracing::info!("rust-analyzer not available"); InvokeResponse::default() }
            Err(e) => { tracing::warn!("rust-analyzer failed: {e}"); InvokeResponse::default() }
        }
    }
}
