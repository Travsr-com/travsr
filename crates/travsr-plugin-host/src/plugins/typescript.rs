...
impl Plugin for TypeScriptPlugin {
    ...
    fn supports_phase_b(&self) -> bool { true } // travsr-lsif-ts
    fn parse(&self, req: &ParseRequest) -> ParseResponse {
        if let Some(src_bytes) = &req.source {
            // git-blob path: write to tempfile then parse
            use std::io::Write;
            let ext = req.path.extension().and_then(|e| e.to_str()).unwrap_or("ts");
            match tempfile::Builder::new().suffix(&format!(".{ext}")).tempfile() {
                Ok(mut tmp) => {
    ...
    fn invoke_phase_b(&self, req: &InvokeRequest) -> InvokeResponse {
        ...
        match travsr_indexer::run_lsif_emitter(&tsconfig) {
            Ok(dump) => match travsr_indexer::ingest_lsif(&dump) {
                Ok(lsif_out) => InvokeResponse { nodes: lsif_out.nodes, edges: lsif_out.edges },
                Err(e) => { tracing::warn!("ts lsif ingest: {e}"); InvokeResponse::default() }
            },
            Err(e) => { tracing::warn!("ts lsif emitter: {e}"); InvokeResponse::default() }
        }
    }
}
