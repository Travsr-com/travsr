...
async fn async_main() {
    ...
    // UX-019: publish the product version so the daemon's session-start log
...
mod daemon_log_tests {
    ...
    use super::{is_entry_start, parse_level, parse_since, Cli, LineFilter, LogLine, LogRenderer};
    ...
    /// `--version` and the daemon's `version=` telemetry field must both report
    /// the crate version. They disagreed before: the daemon logged the workspace
    /// version while `--version` printed the CLI crate's.
    #[test]
    fn release_version_is_reported_consistently() {
        ...
        assert!(
            ...
            "--version must print the crate version, got {rendered:?}"
        );
    }
}
