...
pub fn ingest_scip(path: &Path, corpus: &str) -> anyhow::Result<ParseOutput> {
    let bytes =
        std::fs::read(path).with_context(|| format!("reading SCIP output {}", path.display()))?;
    let index = scip::types::Index::parse_from_bytes(&bytes).context("decoding SCIP protobuf")?;

    let mut nodes = Vec::new();

    for doc in &index.documents {
        let lang_str = doc.language.to_lowercase();
        let Some(language) = Language::from_str(&lang_str) else {
            continue;
        };
        ...
        let mut def_lines: std::collections::HashMap<&str, u32> = std::collections::HashMap::new();
        ...
        for sym in &doc.symbols {
            ...
            let kind = scip_kind_to_node_kind(sym.kind.value());
            if kind == "local" {
