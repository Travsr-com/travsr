...
use travsr_indexer::{
    hash_file, ingest_lsif, link_imports, link_imports_go, link_imports_java,
    link_imports_python_fs, link_imports_rust, run_lsif_emitter, FfiMarker,
};
            ...
            scope.spawn(move || {
                ...
                for abs_path in shard {
                    ...
                    let import_edges = match Language::from_extension(ext) {
                        ...
                        Some(Language::Java) => {
                            link_imports_java(&out.nodes, &corpus, &repo)
                        }
                        _ => vec![],
                    };
