...
- **`travsr lang install` offered Windows wrappers that no travsr-lang release ever shipped (#588).** Every language ended in a raw `404 Not Found` on Windows: `current_target()` returned `x86_64-pc-windows-msvc`, the installer built an asset name from it, and the release matrix had no Windows leg — so `travsr init` offered an interactive setup that could not succeed. Fixed on both sides. travsr-lang now builds and publishes `x86_64-pc-windows-msvc` wrappers, named with `.exe` (plus the matching `.exe.sha256` sidecar), and the installer writes them to `~/.travsr/bin/<name>.exe`. What the release actually ships is now stated in one place (`WRAPPER_RELEASE_TARGETS` / `MACOS_ONLY_WRAPPERS`): anything outside it is refused before any network call, with `travsr lang list`, `travsr lang detect` and `travsr init` reporting "not available on `<target>` yet" instead of offering the install. That gate also covers `travsr-lang-objectivec` on Linux, which 404'd for the same reason. A tag that genuinely lacks an asset now reports the release version and platform rather than a bare URL, and the PATH hint printed after an install uses PowerShell/`setx` syntax on Windows instead of `export`/`~/.zshrc`. A new weekly `lang-release-drift` workflow compares every asset name the installer would request against the live travsr-lang release inventory, so the two repos cannot drift apart silently again.
