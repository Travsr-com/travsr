...
- **The daemon can be stopped from the extension.** The CLI has `daemon stop`, the extension had only Start and Restart, so the one lifecycle action it did not expose was the one that needs a warning. It is a command, `Travsr: Stop Daemon`, reachable from the palette and the status bar menu rather than a button on the Health page: that page is built around remedies, and a one-click path into the "Not watching" state does not belong among them. It confirms first, and says queries keep working while commits and saves stop refreshing the graph.
