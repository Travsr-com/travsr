...
- **The daemon can be stopped from the extension.** The CLI has `daemon stop`, the extension had only Start and Restart, so the one lifecycle action it did not expose was the one that needs a warning. Stop sits in the Health page's Daemon section, next to Restart, and only there: the section that reports the daemon's state is where you look when you want to change it. It appears only while the daemon is running, confirms first, and says queries keep working while commits and saves stop refreshing the graph.
- **The embedding model can be changed from the Sidecars section.** The row reported the active model and offered no way to change it, while the CLI has a catalog and two ways to move between entries. A **Change model** action lists the backends from the `embed list --json` the page already reads, so the picker costs no extra spawn, and it runs the command that fits what is on disk: `embed switch` for an installed model, `embed init --backend` for one that still needs downloading, since switch fails on absent weights. It appears only when there is more than one backend, because a picker with a single entry is a dead end dressed as a choice. Both paths confirm first and say whether a download is involved.
### Fixed

- **Re-index can be cancelled.** `Travsr: Re-index Now` ran under a non-cancellable progress notification while the Health page ran the same `init` under a cancellable one, so whether you could stop a long index depended on which button you pressed. It is cancellable now, and a cancelled run reports itself as cancelled rather than as a failure, since a killed process exits non-zero and calling the user's own cancellation an error is its own small lie.

- **A repository with no graph yet is offered `init` first, not Start daemon.** When nothing answered and there was no graph on disk, the verdict ranked "not answering" above "not indexed" and offered Start daemon, which starts a daemon with nothing to serve and leaves `travsr init` still to do. No graph now outranks everything: the banner reads "No graph yet", says that indexing also starts the daemon, and its action is Index this repo.
- **The Repositories section shows the repository you have open, and says what it actually checked.** Three separate faults met in one section. The rows sorted on whether the graph database was on disk, but a machine that has run the test suite carries a dozen `.tmpXXXXXX` entries whose databases are still there, so they tied with the open repository; the registry arrives sorted by name, `.tmp` sorts ahead of most real names, and the row the whole page is about fell past the eight-row cap. The open repository now sorts first whatever else is registered, live ones next, stale last. The "and N more" line took its stale count over the entire registry while the sentence describes only the rows that were cut, so a mostly-stale registry could claim more missing databases than there were hidden rows; it is counted over the hidden rows now, and reads "and 7 more" rather than "and 7 more, 0 of which have no database" when there is nothing to qualify. And a live row said "ok", which reads as a verdict on the repository when all that was checked is whether `graph.db` is on disk: a checkout deleted months ago said "ok" for as long as its database survived in `~/.travsr`. It says **database present** now, against the **database missing** it was already using, and each row's tooltip carries the database path, since two checkouts can share a basename and the registry shows nothing else to tell them apart.
