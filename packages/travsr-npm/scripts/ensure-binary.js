...
// #576/#691: must stay in lockstep with the release workflow matrix, the
// vscode TARGET_MAP, and install.sh; checked in CI by .github/scripts/check-target-maps.mjs.
const TARGETS = {
...
// Shared by the postinstall script and bin/travsr.js's on-demand
// fallback, so a binary that never got downloaded at install time (npm's
// allow-scripts gate silently skipping the postinstall script, --ignore-scripts,
// a flaky network mid-install) is recoverable on first actual invocation
// instead of a dead-end "binary not found" error.
async function ensureBinary() {
  ...
  // --force-local: without it, GNU tar reads a colon anywhere before the
  // first path separator as a "host:path" remote-tape spec, so an absolute
  // Windows path (C:\...) makes it try to rsh/ssh into a host named "C" and
  // fail with "Cannot connect to ...: resolve failed" instead of extracting.
  // Both GNU tar and bsdtar (macOS's and Windows 10's built-in tar.exe)
  // accept the flag and it is a no-op on a path with no colon.
  const binName = process.platform === 'win32' ? 'travsr.exe' : 'travsr';
  execFileSync('tar', ['-xzf', tmpTar, '--force-local', '-C', BIN_DIR, binName], { stdio: 'inherit' });
...
// npm's .bin shim symlinks to bin/travsr.js, and the kernel ignores the file
...
module.exports = { ensureBinary, BIN_DIR, VERSION };
